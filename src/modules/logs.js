...
// Helpers
// ────────────────────────────────────────────────────────────────────────────────
function safeSend(client, guildId, configKey, embed) {
    if (!guildId)
        return;
    sendDiscordLog(client, guildId, configKey, { embeds: [embed] }).catch((err) => logger.warn('LOGS_MODULE', `sendDiscordLog(${configKey}) threw:`, err));
}
...
export function init(client) {
    ...
    client.on('messageUpdate', (oldMessage, newMessage) => {
        ...
        safeSend(client, guildId, 'message_logs', new EmbedBuilder()
            .setTitle('📝 Mensagem Editada')
            .addFields({ name: 'Autor', value: `${oldMessage.author}`, inline: true }, { name: 'Canal', value: `${oldMessage.channel}`, inline: true }, { name: 'Original', value: `\`\`\`${escapeTicks(oldMessage.content)}\`\`\`` }, { name: 'Nova', value: `\`\`\`${escapeTicks(newMessage.content)}\`\`\`` }, { name: 'ID', value: oldMessage.id, inline: true }, { name: 'Data', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }, {
            name: 'Ir para',
    ...
    client.on('messageDelete', (message) => {
        ...
        if (message.author?.bot)
            return;
        safeSend(client, message.guild?.id, 'message_logs', new EmbedBuilder()
            .setTitle('🗑️ Mensagem Deletada')
            .addFields({ name: 'Autor', value: `${message.author}`, inline: true }, { name: 'Canal', value: `${message.channel}`, inline: true }, { name: 'Conteúdo', value: escapeTicks(message.content) || '*vazia*' })
            .setColor('Red')
            .setFooter({ text: `ID: ${message.id}` }));
    });
    // ── MEMBERS ─────────────────────────────────────────────────────────────────
    client.on('guildMemberAdd', (member) => {
        safeSend(client, member.guild.id, 'member_logs', new EmbedBuilder()
    ...
    client.on('guildBanAdd', async (ban) => {
        ...
        safeSend(client, ban.guild.id, 'member_logs', new EmbedBuilder()
            .setTitle('🔨 Usuário Banido')
            .setDescription(`👤 ${ban.user.tag} (${ban.user.id}) foi banido.`)
            .addFields({ name: 'Motivo', value: ban.reason ?? 'Não especificado', inline: true }, { name: 'Moderador', value: moderador, inline: true })
    ...
    client.on('guildBanRemove', async (ban) => {
        ...
        safeSend(client, ban.guild.id, 'member_logs', new EmbedBuilder()
            ...
            .addFields({ name: 'Usuário', value: `${ban.user.tag} <@${ban.user.id}>`, inline: true }, { name: 'Moderador', value: moderador, inline: true })
            .setThumbnail(ban.user.displayAvatarURL())
            .setColor('Orange')
            .setFooter({ text: `ID: ${ban.user.id}` }));
    });
    client.on('guildMemberUpdate', async (oldMember, newMember) => {
        ...
        if (addedRoles.size || removedRoles.size) {
            ...
            safeSend(client, newMember.guild.id, 'member_logs', new EmbedBuilder()
        ...
        if (oldMember.nickname !== newMember.nickname) {
            ...
            safeSend(client, newMember.guild.id, 'member_logs', new EmbedBuilder()
                ...
                .setDescription(`<@${newMember.id}> (${newMember.user.tag})`)
                .addFields({ name: 'Apelido antigo', value: oldMember.nickname ?? oldMember.user.username, inline: true }, { name: 'Apelido novo', value: newMember.nickname ?? newMember.user.username, inline: true }, { name: 'Responsável', value: moderador })
                .setThumbnail(newMember.user.displayAvatarURL())
                .setColor('Blue')
                .setFooter({ text: `ID: ${newMember.id}` }));
        }
    ...
    client.on('roleUpdate', async (oldRole, newRole) => {
        ...
        safeSend(client, newRole.guild.id, 'role_logs', new EmbedBuilder()
    ...
    client.on('roleCreate', async (role) => {
        ...
        safeSend(client, role.guild.id, 'role_logs', new EmbedBuilder()
            .setTitle('🆕 Cargo Criado')
            .addFields({ name: 'Nome', value: role.name, inline: true }, { name: 'ID', value: role.id, inline: true }, { name: 'Criado por', value: moderador, inline: true })
            .setColor('Green')
    ...
    client.on('roleDelete', async (role) => {
        ...
        safeSend(client, role.guild?.id, 'role_logs', new EmbedBuilder()
            ...
            .addFields({ name: 'Nome', value: role.name, inline: true }, { name: 'ID', value: role.id, inline: true }, { name: 'Deletado por', value: moderador, inline: true })
            .setColor('Red')
            .setFooter({ text: `ID: ${role.id}` }));
    });
    // ── CHANNELS ─────────────────────────────────────────────────────────────────
    client.on('channelCreate', async (channel) => {
        ...
        safeSend(client, channel.guild.id, 'channel_logs', new EmbedBuilder()
            .setTitle('📢 Canal Criado')
            .addFields({ name: 'Nome', value: channel.name, inline: true }, { name: 'ID', value: channel.id, inline: true }, { name: 'Criado por', value: moderador, inline: true })
            .setColor('Green')
    ...
    client.on('channelDelete', async (channel) => {
        ...
        safeSend(client, channel.guild.id, 'channel_logs', new EmbedBuilder()
            ...
            .addFields({ name: 'Nome', value: channel.name, inline: true }, { name: 'ID', value: channel.id, inline: true }, { name: 'Deletado por', value: moderador, inline: true })
            .setColor('Red')
            .setFooter({ text: `ID: ${channel.id}` }));
    });
    client.on('channelUpdate', async (oldChannel, newChannel) => {
        ...
        if ('name' in oldChannel && 'name' in newChannel && oldChannel.name !== newChannel.name) {
            ...
            safeSend(client, guild.id, 'channel_logs', new EmbedBuilder()
                .setTitle('✏️ Canal Renomeado')
                .addFields({ name: 'Nome antigo', value: oldChannel.name, inline: true }, { name: 'Nome novo', value: newChannel.name, inline: true }, { name: 'Moderador', value: moderador, inline: true })
                .setColor('Blue')
        ...
        if (lines.length) {
            ...
            if (permsText.length > 1024)
                permsText = permsText.slice(0, 1021) + '...';
            ...
            safeSend(client, guild.id, 'channel_logs', new EmbedBuilder()
                .setTitle(`🏠 Permissões do Canal Atualizadas: ${'name' in newChannel ? newChannel.name : newChannel.id}`)
                .addFields({ name: 'Permissões', value: permsText || '—' }, { name: 'Moderador', value: moderador })
                .setColor('Blue')
                .setFooter({ text: `ID: ${newChannel.id}` }));
        }
    });
    ...
    client.on('guildUpdate', async (oldGuild, newGuild) => {
        ...
        if (oldGuild.name !== newGuild.name)
            changes.push(`✏️ Nome: **${oldGuild.name}** → **${newGuild.name}**`);
        if (oldGuild.icon !== newGuild.icon)
            changes.push(`🖼️ Ícone alterado.`);
        if (oldGuild.banner !== newGuild.banner)
            changes.push(`🖼️ Banner alterado.`);
        if (!changes.length)
            return;
        const moderador = await fetchModerator(newGuild, AuditLogEvent.GuildUpdate, newGuild.id);
        safeSend(client, newGuild.id, 'guild_logs', new EmbedBuilder()
    ...
    client.on('userUpdate', (oldUser, newUser) => {
        ...
        client.guilds.cache.forEach((g) => {
            safeSend(client, g.id, 'user_logs', new EmbedBuilder()
                .setTitle('👤 Usuário Renomeado')
                .addFields({ name: 'Nome antigo', value: oldUser.username ?? '—', inline: true }, { name: 'Nome novo', value: newUser.username ?? '—', inline: true })
