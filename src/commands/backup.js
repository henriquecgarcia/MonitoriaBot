...
async function restoreBackup(interaction, fileContent) {
    ...
    for (const cfg of backupData.tables.configs) {
        await db.query('INSERT INTO configs (guild_id, config_key, config_value, updated_at) VALUES (?, ?, ?, ?)', [cfg.guild_id, cfg.config_key, cfg.config_value, cfg.updated_at]);
    }
    for (const ticket of backupData.tables.tickets) {
        await db.query('INSERT INTO tickets (ticket_id, guild_id, channel_id, ticket_type, author_id, assigned_id, status, created_at, closed_at, closed_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
...
let currentPage = 1;
async function listBackups(interaction, page) {
    ensureBackupDir();
    ...
    async execute(interaction, _ctx) {
        ...
        try {
            if (sub === 'create')
                ...
                await restoreBackup(interaction, interaction.options.getString('file', true));
            else if (sub === 'list')
                await listBackups(interaction, 1);
            else if (sub === 'delete')
                await deleteBackup(interaction, interaction.options.getString('file', true));
        }
        catch (err) {
