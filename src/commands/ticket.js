...
import { createTicket, closeTicket, getAllTicketTypes } from '../services/tickets.js';
...
const command = {
    ...
    async execute(interaction, { client }) {
        ...
        if (sub === 'abrir') {
            ...
            try {
                ...
                if (msg === 'ERR_RATE_LIMIT')
                    return interaction.editReply({ content: '❌ Você está criando tickets rápido demais.' });
                if (msg === 'ERR_MISSING_CATEGORY')
                    return interaction.editReply({ content: '❌ Categoria de ticket não configurada.' });
        ...
        if (sub === 'fechar') {
            ...
            if (!staffPerms)
                return interaction.editReply({ content: '❌ Sem permissão para fechar tickets.' });
        ...
        if (sub === 'assumir') {
            ...
            if (!staffPerms)
                return interaction.editReply({ content: '❌ Sem permissão para assumir tickets.' });
            if (ticket.assigned_id)
                return interaction.editReply({ content: '❌ Este ticket já está atribuído.' });
        ...
        if (sub === 'mensagem') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription('Para abrir um ticket, selecione o tipo no menu abaixo. Um canal privado será criado para você e a equipe de suporte.')
                .setColor('Blue')
            ...
            const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .addOptions(options));
            await interaction.editReply({ content: '✅ Mensagem postada.', ephemeral: true });
            await channel.send({
