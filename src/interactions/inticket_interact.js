...
import { assignTicket, canClaimTicket } from '../services/tickets.js';
...
const adminMenu = {
    ...
    async execute(interaction, { client: _client }) {
        ...
        const actions = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('add_user_ticket').setLabel('Adicionar Usuário').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('remove_user_ticket').setLabel('Remover Usuário').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('close_ticket').setLabel('Fechar Ticket').setStyle(ButtonStyle.Danger));
        const embed = new EmbedBuilder()
            .setTitle(`Menu Admin do Ticket #${ticket.ticket_id}`)
            .setDescription('Use os botões abaixo para gerenciar este ticket.')
            .addFields({ name: 'Criado por', value: `<@${ticket.author_id}>`, inline: true }, { name: 'Abertura', value: formatDate(ticket.created_at), inline: true }, { name: 'Status', value: ticket.status === 'open' ? 'Aberto' : 'Fechado', inline: true }, { name: '\u200B', value: '\u200B' }, { name: 'Tipo', value: ticket.ticket_type, inline: true }, { name: 'Responsável', value: ticket.assigned_id ? `<@${ticket.assigned_id}>` : 'Ninguém', inline: true }, { name: 'Canal', value: `<#${channel.id}>`, inline: true })
            .setColor(0x00ae86)
            .setTimestamp()
...
const removeUser = {
    ...
    async execute(interaction, _ctx) {
        ...
        const extraMembers = guild.members.cache.filter((m) => !m.user.bot &&
