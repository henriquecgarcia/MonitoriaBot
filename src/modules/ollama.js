...
export async function init(client) {
    ...
    client.on(Events.MessageCreate, async (message) => {
        ...
        if (!await isEnabled(message.guild.id))
            return;
        const channelId = await config.get(message.guild.id, 'ollama_channel');
        if (!channelId)
            return;
        ...
        const configuredModel = await config.get(message.guild.id, 'ollama_model');
        const configuredContextSize = await config.get(message.guild.id, 'ollama_context_size');
        const contextSize = typeof configuredContextSize === 'number'
