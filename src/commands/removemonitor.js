...
const command = {
    data: new SlashCommandBuilder()
        ...
        .setRequired(true)
        .addChoices({ name: 'LP', value: 'LP' }, { name: 'AEDI', value: 'AEDI' }, { name: 'AEDII', value: 'AEDII' })),
    async execute(interaction) {
        ...
        if (!member.roles.cache.has(monitorRole.id)) {
            return interaction.editReply({ content: '❌ Esse usuário não é um monitor dessa disciplina.' });
        }
        await member.roles.remove(monitorRole);
        return interaction.editReply({ content: `✅ Monitor ${monitor} da disciplina ${disciplina} removido com sucesso!` });
    },
};
