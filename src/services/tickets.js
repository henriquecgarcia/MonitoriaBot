...
export function getTicketType(_guildId, typeKey) {
...
const lastTicketAt = new Map();
const pendingTicketCreations = new Set(); // key = `${guildId}-${userId}`
function getRateLimitKey(guildId, userId) {
...
function markRateLimit(guildId, userId) {
    lastTicketAt.set(getRateLimitKey(guildId, userId), Date.now());
}
function clearRateLimit(guildId, userId) {
...
async function _createTicketInternal({ guild, user, type, client }) {
    ...
    // Resolve category
    const catId = (await config.get(guild.id, ticketType.categoryConfig));
    if (!catId) {
    ...
    const staffRoles = (await config.get(guild.id, ticketType.roleIdConfig));
    ...
    const placeholder = `Este canal é um ticket do tipo **${ticketType.name}** aberto por <@${user.id}>.\n` +
    ...
    const pingText = (staffRoles?.length ? `<@&${staffRoles[0]}>` : '') +
    ...
    const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('close_ticket').setLabel('Fechar Ticket').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('claim_ticket').setLabel('Assumir Ticket').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('admin_menu_ticket').setLabel('Menu Admin').setStyle(ButtonStyle.Secondary));
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: 'Tipo de Ticket', value: ticketType.name, inline: true }, { name: 'Criado por', value: `<@${user.id}>`, inline: true }, { name: 'Status', value: 'Aberto', inline: true });
            ...
            await logCh
                ...
                embeds: [
                    new EmbedBuilder()
                        ...
                        .addFields({ name: 'Canal', value: `<#${channel.id}>`, inline: true }, { name: 'Autor', value: `<@${user.id}>`, inline: true }, { name: 'Tipo', value: type, inline: true })
                        .setColor(0x57f287)
                        .setTimestamp(),
                ],
            })
...
// assignTicket
// ────────────────────────────────────────────────────────────────────────────────
export async function assignTicket({ channel, assignedTo }) {
    const guild = channel.guild;
    const ticketData = await db.getTicketByChannel(guild.id, channel.id);
    if (!ticketData)
        throw new Error('ERR_NOT_A_TICKET');
...
export async function closeTicket({ channel, closedBy, client, opts = {} }) {
    ...
    if (!logTranscriptId)
        throw new Error('ERR_MISSING_LOG_TRANSCRIPT');
    const logCh = await client.channels.fetch(logTranscriptId).catch(() => null);
    if (!logCh?.isTextBased())
        throw new Error('ERR_LOG_CHANNEL_NOT_FOUND');
    // Generate HTML transcript
    let transcript = null;
    try {
        const result = await discordTranscript.createTranscript(channel, {
            limit: -1,
        ...
        logger.warn('TICKET_CLOSE', 'Falha ao gerar transcript HTML, ignorando:', err.message);
    }
    await logCh.send({
    ...
    logger.info('TICKET_CLOSE', `Ticket #${ticketData.ticket_id} (${ticketData.ticket_type}) fechado por ${closedBy.tag} (${closedBy.id}) no guild ${guild.id}`);
    // Rename channel
