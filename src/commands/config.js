...
function extractRoleIds(input) {
    const ids = new Set();
    for (const m of input.matchAll(/<@&(?<id>\d+)>/g))
        if (m.groups?.id)
            ids.add(m.groups.id);
    for (const m of input.matchAll(/\b(\d{5,})\b/g))
        if (m[1])
            ids.add(m[1]);
    return [...ids];
}
...
const command = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sc) => sc.setName('set-roles').setDescription('Define múltiplos cargos para uma chave')
        ...
        .addSubcommand((sc) => sc.setName('set-category').setDescription('Define uma categoria para tickets')
        ...
        .addSubcommand((sc) => sc.setName('set-bool').setDescription('Define um valor booleano')
        ...
        .addSubcommand((sc) => sc.setName('set-string').setDescription('Define um valor de texto')
    ...
    async execute(interaction, _ctx) {
        ...
        try {
            ...
            if (sub === 'set-roles') {
                ...
                if (!ids.length)
                    return interaction.editReply({ embeds: [errorEmbed('Entrada inválida', 'Use menções (<@&id>) ou IDs separados por vírgula.')] });
            ...
            if (sub === 'list') {
                ...
                const embed = infoEmbed('Chaves configuráveis', 'Use o subcomando correto para alterar cada tipo de chave.')
                    .addFields({ name: 'Canais', value: fmt(config.keysByType(config.TYPES.CHANNEL)) }, { name: 'Categorias', value: fmt(config.keysByType(config.TYPES.CATEGORY)) }, { name: 'Cargo (único)', value: fmt(config.keysByType(config.TYPES.ROLE)) }, { name: 'Cargos (múltiplos)', value: fmt(config.keysByType(config.TYPES.ROLES)) }, { name: 'Booleanos', value: fmt(config.keysByType(config.TYPES.BOOL)) }, { name: 'Strings', value: fmt(config.keysByType(config.TYPES.STRING)) });
